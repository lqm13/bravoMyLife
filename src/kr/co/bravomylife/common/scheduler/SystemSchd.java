 ...
 *				: [20241028115342][pluto@himedia.co.kr][CREATE: Initial Release]
 */
package kr.co.bravomylife.common.scheduler;
...
public class SystemSchd {
	...
	*/
	@Scheduled(cron = "0 0 * * * *")
	public void schedule_best() {
		
		try {
			
			saleSrvc.schedule_best();
			
			// Calendar calendar = Calendar.getInstance();
			// SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
			// logger.debug("[" + this.getClass().getName() + ".schedule_best()] " + "Run the scheduler every 10 seconds: " + dateFormat.format(calendar.getTime()));
		}
		catch (Exception e) {
			logger.error("[" + this.getClass().getName() + ".schedule_best()] " + e.getMessage(), e);
		}
	}
}
